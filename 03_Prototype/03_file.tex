...
\begin{refsection}
  ...
  It is important to underline that designing \acrshort{ipm}s to work in the requested conditions is really challenging. Preliminary studies were done in order to check the feasibility of the \acrshort{ipm} design. We focussed our efforts on the following hot topics:
  \begin{itemize}
  ...
  \begin{itemize}
    ...
    \item Heed ++: a C ++ code that implements an ionization model based on the photoabsorption ionization (PAI) \cite{Allison1980,Smirnov2005}.
  \end{itemize}
  ...
  The Table \ref{chap3:GarfieldBethe} reports the value of expected number of electrons/ions pairs per cm obtain from the direct calculation (Bethe equation) and from the results of the Garfield++ simulations. One can see that the Garfield++ value is always lower than the calculation by a constant factor X.
  ...
  \end{equation}
  Where $Id$ is the identity matrix, $v_{u}$ unknown values of upper line, $v_{d}$ for the lower line, $v_{c}$ for the current, $ A =
    \begin{pmatrix}
      -4     & 1      & 0      & \cdots \\
      1      & -4     & 1      & \cdots \\
      0      & 1      & -4     & \cdots \\
  ...
  Detection conductive strips because it is the simplest and most robust method to implement. Unfortunately, the sensitivity is limited. When the signal is too weak, it is necessary to add an amplifier like Micro Channel Plates. Finally, we also considered a solution based on semiconductor detectors. This novel method is developed by CERN and shows promising results.
  
  In this section the operating principle of these three methods is described as well as their advantages and drawbacks.
  ...
  Conductive strips is the simplest method to implement. Electrodes are etched on a PCB with a thin layer of copper. 
